...
class ApiHelper {
  ...
  static Future<TvShowsResponse> getPopularTvShows() async {
    var res = await http.get(
        Uri.parse(Constants.TvShowBaseUrl + "/popular" + Constants.apiKey));
    if (res.statusCode == 200) {
  ...
  static Future<MoviesResponse> findMovies(String query) async {
    var res = await http.get(Uri.parse(Constants.movieSearchBase +
        Constants.apiKey +
        Constants.queryPrefix +
        query));
    if (res.statusCode == 200) {
      return MoviesResponse.fromJson(json.decode(res.body));
    } else {
  ...
  static Future<TvShowsResponse> findTvShows(String query) async {
    var res = await http.get(Uri.parse(Constants.tvShowSearchBase +
        Constants.apiKey +
        Constants.queryPrefix +
        query));
    if (res.statusCode == 200) {
